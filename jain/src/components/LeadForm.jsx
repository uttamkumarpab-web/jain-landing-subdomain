...
export default function LeadForm() {
  ...
  const updateField = (field, sanitizer) => ({
    target: { value },
  }) => {
  ...
  return (
    <div className="bg-white text-gray-800 rounded-3xl shadow-lg px-6 py-4 max-sm:px-5 max-sm:py-6">
      <h2 className="text-center font-bold mb-2 text-[25px] text-[#194486]">
        Download Brochure for Online MBA
      </h2>
      <p className="text-center text-sm text-[#4A5565] mb-3">
        Get information about programs from JAIN University
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-1" noValidate>
        ...
        <div className="inline-flex justify-center items-center w-full mt-3">
          <button
            type="submit"
            disabled={submitting}
            className="w-[250px] max-sm:w-full bg-[#194486] text-white font-medium py-2 px-4 rounded transition duration-300 cursor-pointer disabled:opacity-70"
          >
            <svg
